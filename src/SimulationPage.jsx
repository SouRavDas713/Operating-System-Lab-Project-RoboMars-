...
const SimulationPage = ({ tasks, algorithm, onBack }) => {
  ...
  const getAverageWaitingTime = () => {
    const finished = taskList.filter((t) => t.startTime !== null);
    if (finished.length === 0) return "-";
    const total = finished.reduce(
      (sum, t) => sum + (t.startTime - t.arrivalTime),
      0
    );
    ...
    <div className="flex flex-col h-screen bg-linear-to-br from-gray-900 via-gray-800 to-gray-700 p-4">
            ...
            <tbody>
              ...
              {taskList.length === 0 && (
                <tr>
                  <td
                    colSpan={7}
                    className="py-3 text-gray-400 text-xs italic"
                  >
                    No tasks added.
                  </td>
                </tr>
      ...
      {analyticsOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center">
          <div className="bg-gray-900 p-6 rounded-2xl w-3/4 max-h-[80vh] overflow-y-auto text-white shadow-2xl border border-gray-700">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-2xl font-bold mb-1">📊 CPU Analytics</h2>
                <p className="text-sm text-gray-300">
                  Algorithm used:{" "}
                  <b>{algorithm || "Preemptive Priority Scheduling"}</b>
                </p>
              </div>
              <button
                onClick={toggleAnalytics}
                className="bg-gray-800 px-3 py-1 rounded-lg text-sm hover:bg-gray-700"
              >
                ✕ Close
              </button>
            </div>

            {/* ✅ New summary row for averages */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4 text-sm">
              <div className="bg-gray-950 rounded-xl p-3 border border-gray-700">
            ...
            <table className="min-w-full text-center border border-gray-700 text-xs">
              <thead className="bg-gray-800">
                <tr>
                  <th className="py-2 px-2 border-b">Task Name</th>
                  <th className="py-2 px-2 border-b">Priority</th>
                  <th className="py-2 px-2 border-b">Arrival</th>
                  <th className="py-2 px-2 border-b">Burst</th>
                  <th className="py-2 px-2 border-b">Waiting</th>
                  <th className="py-2 px-2 border-b">Turnaround</th>
                </tr>
              </thead>
              <tbody>
                {taskList.map((t, i) => (
                  ...
                  >
                    ...
                    <td className="py-1 px-2 border-b">{t.priority}</td>
                    <td className="py-1 px-2 border-b">{t.arrivalTime}</td>
                    <td className="py-1 px-2 border-b">{t.burstTime}</td>
            ...
            <p className="mt-3 text-[11px] text-gray-400">
              * A task is included in average waiting time once it has started,
              and in average turnaround time once it has completed.
            </p>
          </div>
        </div>
      )}
